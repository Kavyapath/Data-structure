...
void updateedge(int sourcev,int destinationv,int nweight){
     ...
     void deletevertex(int sourcev){
        ...
        for(auto i=vertices.at(vindex).edgelist.begin();i!=vertices.at(vindex).edgelist.begin();i++){
            deleteedge(i->getdestination(),sourcev);
        }
        vertices.erase(vertices.begin()+vindex);
        cout<<"vertex deleted"<<endl;
     }
     void updatevertex(int vid,string vname){
        bool check=checkvertexexist(vid);
        if(check==true){
             for(int i=0;i<vertices.size();i++){
                if(vertices.at(i).getstateid()==vid){
